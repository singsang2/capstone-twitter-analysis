...
def create_table():
    c.execute("CREATE TABLE IF NOT EXISTS sentiment(unix REAL, id TEXT, user TEXT, tweet TEXT, clean TEXT, favorite INT, retweet INT, sentiment REAL, proba REAL)")
    conn.commit()
create_table()
...
class listener(StreamListener):
    def on_data(self, data):
        try:
            ...
            proba = predictor.predict_proba([clean])[0]
            proba = -float(proba[0]) if proba[0] > proba[1] else float(proba[1])
            # Sentiment Analysis *Change model if necessarity
            sentiment = TextBlob(tweet).sentiment.polarity
            print(time_ms, tweet, sentiment, proba)
            
            c.execute("INSERT INTO sentiment (unix, id, user, tweet, clean, favorite, retweet, sentiment, proba) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                      (time_ms, id_str, user_str, tweet, clean, favorite, retweet, sentiment, proba))
            conn.commit()
